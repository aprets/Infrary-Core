...
VMConfiguration = {"isMaster": True ,"selfDestruct": True ,"cmdList": ["curl https://releases.rancher.com/install-docker/17.06.sh | sh","service ntp stop","update-rc.d -f ntp remove","fallocate -l 4G /swapfile", "chmod 600 /swapfile","mkswap /swapfile", "swapon /swapfile","echo \"/swapfile   none    swap    sw    0   0\" >> /etc/fstab" ,"docker run -d --restart=unless-stopped -p 8080:8080 rancher/server", "sleep 60"]}
